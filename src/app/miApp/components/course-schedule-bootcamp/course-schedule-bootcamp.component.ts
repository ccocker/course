...
@Component({
  ...
  templateUrl: './course-schedule-bootcamp.component.html',
  styleUrl: './course-schedule-bootcamp.component.scss',
})
export class CourseScheduleBootcampComponent implements OnInit, OnDestroy {
...
*/
  ...
  events = events
  // rooms = rooms;
  filteredEvents$: Observable<IScheduleEvent[]>
  constructor(
  ...
  ngOnInit() {
    ...
    this.events$ = this.store.select(selectEventsEntities).pipe(
      tap((events) => console.log('Events received:', events)), // Debugging line
      startWith([]), // Ensures `this.events$` has a default value of an empty array
    )

    // this.filteredEvents$ = combineLatest([
    //   this.store.select(selectCurrentUser).pipe(
    //     tap((user) => console.log('Current user:', user)) // Debugging line
    //   ),
    //   this.events$,
    // ]).pipe(
    //   map(([currentUser, events]) =>
    //     this.filterEventsByCurrentUser(events, currentUser)
    //   ),
    //   tap((filteredEvents) => console.log('Filtered events:', filteredEvents)) // Debugging line
    // );
  ...
  filterEventsByCurrentUser(
    ...
    // Log the input parameters to check their values
    console.log('Filtering events for currentUser:', currentUser)
    console.log('Total events to filter:', events.length)
    console.log('Total events to filter:', events)

    const filteredEvents = events.filter(
      (event) =>
        event['leadName'] === currentUser.displayName ||
        (event['tutors'] || []).includes(currentUser.displayName),
    )

    // Log the filtered events to see what's being returned
    console.log('Filtered events count:', filteredEvents.length)
    if (filteredEvents.length === 0) {
      console.log(
        'No events match the current user criteria. Check the currentUser.displayName and event data structure.',
      )
    }
  ...
  shouldDisplayEvent(event: IScheduleEvent): boolean {
    ...
    const isAnyStaffSelected =
      event.class['staff']?.some(
        (staff) => this.selectedStaff?.includes(staff.miId),
      ) || '1'
